"""
AI Verification-Duplication Service for RiskRadar

This service provides ML-based verification of incident reports:
- Fake report detection using fine-tuned RoBERTa model
- Duplicate report detection using BERT embeddings + classifier

Key principle: AI never auto-rejects reports - only flags for moderator review.
"""
...
# =============================================================================
bert_model = None  # RoBERTa model for text embedding extraction
tokenizer = None  # RoBERTa tokenizer
scaler = None  # Feature scaler (loaded but unused - dimension mismatch)
fake_detector = None  # Neural network classifier head for fake detection
duplicate_classifier = None  # Sklearn classifier for duplicate detection
...
class VerificationRequest(BaseModel):
    """Request payload for fake report verification."""
...
class VerificationResponse(BaseModel):
    """Response with fake detection results. Note: should_reject is always False."""

    report_id: str
    is_fake: bool  # True = suspicious, needs moderator review
    fake_probability: float  # 0.0-1.0 probability score
    confidence: str  # "high", "medium", or "low"
    should_reject: bool = False  # Always False - AI never auto-rejects
    explanation: Optional[str] = None  # User-friendly message


class DuplicateCheckRequest(BaseModel):
    """Request payload for duplicate report detection."""

    report_id: str = Field(..., description="Report ID to check")
    title: str = Field(..., description="Report title")
    description: str = Field(..., description="Report description")
    user_id: str = Field(..., description="User ID")
    existing_reports: List[Dict[str, str]] = Field(
        ..., description="Existing reports to compare against (max 10 used)"
    )
...
class DuplicateCheckResponse(BaseModel):
    ...
    report_id: str
    is_duplicate: bool  # True if max_similarity > 0.85
    duplicate_probability: float  # Highest similarity score found
    similar_reports: List[Dict[str, Any]]  # Reports with similarity > 0.7
...
def check_lfs_pointer(file_path: str) -> bool:
    """Check if file is a Git LFS pointer (not actual model data)."""
    try:
...
def load_models():
    """
    Load all required ML models from detector_model_components/.

    Raises RuntimeError if any model file is missing or corrupted.
    Models must be downloaded via Git LFS before running.
    """
    ...
    if not os.path.exists(bert_path):
        raise RuntimeError(
            f"Required model file not found: {bert_path}. Run 'git lfs pull' to download model files."
        )
    ...
    logger.info(
        f"Detected model config: vocab_size={vocab_size}, max_position={max_position}"
    )
    ...
    # Load scaler (legacy compatibility - not currently used)
    # Note: Scaler expects 4 features but BERT outputs 768-dim embeddings
    scaler_path = os.path.join(model_dir, "scaler.joblib")
    if not os.path.exists(scaler_path):
        raise RuntimeError(f"Required model file not found: {scaler_path}")
    scaler = joblib.load(scaler_path)
    logger.info("✅ Scaler loaded (not used in current pipeline)")
...
def extract_features(text: str) -> np.ndarray:
    """
    Extract 768-dimensional embedding from text using RoBERTa CLS token.

    Args:
        text: Input text to encode

    Returns:
        numpy array of shape (1, 768) containing the embedding
    """
...
async def handle_report_message(
    message: Dict[str, Any], topic: str, partition: int, offset: int
):
    """Process incoming report events from Kafka 'report' topic."""
    logger.info(
        f"Received verification request from topic {topic} (partition {partition}, offset {offset})"
    )
    ...
    try:
        ...
        if status in ["VERIFIED", "REJECTED"]:
            logger.info(
                f"Skipping verification for report {message.get('id')} with status {status}"
            )
            return
...
app = FastAPI(
    ...
    description="ML-based fake and duplicate report detection for RiskRadar",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def health_check():
    ...
    models_ok = all(
        [
            ...
            fake_detector is not None,
            duplicate_classifier is not None,
        ]
    )
...
async def verify_report(request: VerificationRequest):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            logger.info(
                f"Raw probabilities - authentic: {authentic_prob:.4f}, fake: {fake_prob:.4f}"
            )

        is_fake = fake_prob > 0.5
        confidence = (
            "high"
            if abs(fake_prob - 0.5) > 0.3
            else "medium"
            if abs(fake_prob - 0.5) > 0.15
            else "low"
        )
        ...
        logger.info(
            f"Verification result for {request.report_id}: is_fake={is_fake}, fake_prob={fake_prob:.4f}, authentic_prob={authentic_prob:.4f}, confidence={confidence}"
        )
        ...
        try:
            ...
            logger.warning(
                f"Skipping notification for non-UUID user_id: {request.user_id}"
            )
    ...
    try:
        ...
        for existing in request.existing_reports[:10]:  # Limit to 10 comparisons
            ...
            combined_features = np.concatenate(
                [new_features, existing_features], axis=1
            )
            similarity = float(
                duplicate_classifier.predict_proba(combined_features)[0][1]
            )
        ...
        logger.info(
            f"Duplicate check for {request.report_id}: is_duplicate={is_duplicate}, max_similarity={max_similarity:.4f}"
        )
