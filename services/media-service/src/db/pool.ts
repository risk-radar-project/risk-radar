...
class Database {
    ...
    async waitForConnection(
        maxRetries = config.db.bootstrapRetries,
        retryInterval = config.db.bootstrapDelayMs,
        maxWaitMs = config.db.bootstrapMaxWaitMs
    ): Promise<void> {
...
export const db = {
    ...
    waitForConnection: (max?: number, interval?: number, maxWaitMs?: number) =>
        database.waitForConnection(max, interval, maxWaitMs),
    getClient: () => database.getClient(),
    getBootstrapInfo: () =>
        (database as any).getBootstrapInfo?.() || { attempts: 0, lastError: null, startedAt: null, isConnected: false }
}
