...
export const uploadBodySchema = Joi.object({
    visibility: Joi.string().valid(...visibilityEnum).optional(),
    alt: Joi.string().trim().max(config.limits.altMaxLen).optional(),
    temporary: Joi.alternatives().try(
        Joi.boolean(),
        Joi.string().valid('1', '0', 'true', 'false', 'yes', 'no', 'on', 'off')
    ).optional(),
...
export const listQuerySchema = Joi.object({
    ...
    status: Joi.string().valid(...moderationStatusEnum).optional(),
    visibility: Joi.string().valid(...visibilityEnum).optional(),
...
export const patchBodySchema = Joi.object({
    ...
    censor: Joi.object({
        strength: Joi.number().integer().min(config.censor.minStrength).max(config.censor.maxStrength)
    }),
}).min(1).unknown(false);
