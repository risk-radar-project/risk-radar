...
const dbQuery = jest.fn(async (sql: string, params?: any[]) => {
    ...
    if (sql.startsWith('UPDATE media_assets SET is_temporary=false')) {
        ...
        for (const id of ids) {
            ...
            if (rec) { rec.is_temporary = false; rec.expires_at = null; rec.updated_at = new Date(); kept.push({ id }); }
        }
    ...
    if (sql.startsWith('UPDATE media_assets SET deleted=true, deleted_at=now(), updated_at=now() WHERE id=')) {
        ...
        if (rec) { rec.deleted = true; rec.deleted_at = new Date(); rec.updated_at = new Date(); }
        return { rows: [] } as any;
    }
    if (sql.startsWith('UPDATE media_assets SET deleted=true')) {
        ...
        for (const id of ids) {
            const rec = dbState.find(r => r.id === id);
            if (rec && !rec.deleted) { rec.deleted = true; rec.deleted_at = new Date(); rec.updated_at = new Date(); rej.push({ id }); }
        }
...
jest.mock('../../src/moderation/openai-moderation.js', () => ({ moderateImage: jest.fn(async () => ({ flagged: false, elapsedMs: 5 })) }));
