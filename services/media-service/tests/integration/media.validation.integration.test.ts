    ...
    db: {
        query: async (sql: string, params?: any[]) => {
            if (sql.startsWith('INSERT INTO media_assets')) {
                const rec = { id: params![0], owner_id: params![1], visibility: params![2], status: params![3], deleted: false, alt: params![10] };
                dbState.push(rec); return { rows: [rec] } as any;
            }
            if (sql.startsWith('SELECT * FROM media_assets WHERE id=')) { return { rows: dbState.filter(r => r.id === params![0]) }; }
            return { rows: [] };
        }
    }
}));
