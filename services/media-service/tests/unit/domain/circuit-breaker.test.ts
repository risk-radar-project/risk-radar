...
describe('CircuitBreaker', () => {
    test('opens after threshold failures and blocks calls', async () => {
        ...
        await expect(br.exec(async () => { throw new Error('fail'); })).rejects.toThrow('fail');
        await expect(br.exec(async () => { throw new Error('fail'); })).rejects.toThrow('fail');
        // now open
    ...
    test('half-open after cooldown then success closes', async () => {
        ...
        await expect(br.exec(async () => { throw new Error('boom'); })).rejects.toThrow('boom');
    ...
    test('failure during half-open resets to open', async () => {
        ...
        await expect(br.exec(async () => { throw new Error('x'); })).rejects.toThrow('x');
        await sleep(25);
        // first probe fails -> stays open
        await expect(br.exec(async () => { throw new Error('again'); })).rejects.toThrow('again');
