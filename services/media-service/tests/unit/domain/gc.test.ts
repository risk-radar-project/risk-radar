...
jest.mock('../../../src/storage/fs-storage.js', () => ({ deleteFiles: (...args: any[]) => (deleteFiles as any)(...args) }));
jest.mock('../../../src/logger/logger.js', () => ({ logger: loggerMock }));
jest.mock('../../../src/db/pool.js', () => ({
    db: {
        query: jest.fn(async (sql: string, params?: any[]) => {
            queryCalls.push({ sql, params });
            if (sql.startsWith('SELECT id FROM media_assets WHERE deleted=true')) return { rows: [{ id: 'old-deleted-1' }] } as any;
            if (sql.startsWith('SELECT id FROM media_assets WHERE is_temporary=true')) return { rows: [{ id: 'expired-temp-1' }] } as any;
...
describe('gc', () => {
    beforeAll(() => { config.gc.enabled = true as any; });
    beforeEach(() => { queryCalls = []; deleteFiles.mockClear(); });
